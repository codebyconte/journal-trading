...
function Callout({ type = 'info', title, children }: {
  ...
  const v = {
    info:    { border: 'border-accent/30',  bg: 'bg-accent/5',    icon: <Info      size={16} className="text-accent  flex-shrink-0 mt-0.5" />, tc: 'text-accent'  },
    warning: { border: 'border-neutral/40', bg: 'bg-neutral-dim', icon: <AlertTriangle size={16} className="text-neutral flex-shrink-0 mt-0.5" />, tc: 'text-neutral' },
    danger:  { border: 'border-loss/40',    bg: 'bg-loss-dim',    icon: <AlertOctagon  size={16} className="text-loss    flex-shrink-0 mt-0.5" />, tc: 'text-loss'    },
    success: { border: 'border-profit/30',  bg: 'bg-profit-dim',  icon: <CheckCircle2  size={16} className="text-profit  flex-shrink-0 mt-0.5" />, tc: 'text-profit'  },
    tip:     { border: 'border-accent/30',  bg: 'bg-accent/5',    icon: <Lightbulb     size={16} className="text-accent  flex-shrink-0 mt-0.5" />, tc: 'text-accent'  },
  }[type]
  return (
    <div className={cn('flex gap-3 rounded-xl border px-4 py-4', v.border, v.bg)}>
      {v.icon}
      <div className="flex-1 space-y-1">
        {title && <p className={cn('font-semibold text-sm', v.tc)}>{title}</p>}
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return <h2 className="text-xl font-bold text-text-primary border-b border-border pb-3 mb-6">{children}</h2>
}
...
function SubHeading({ icon, children }: { icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5 mt-8 mb-4">
      ...
      <h3 className="text-base font-bold text-text-primary whitespace-nowrap">{children}</h3>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-bg-surface border-b border-border">
            {headers.map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-text-secondary">{h}</th>)}
          </tr>
        </thead>
        <tbody className="divide-y divide-border">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-bg-hover transition-colors">
              {row.map((cell, j) => <td key={j} className="px-4 py-3 text-sm text-text-secondary leading-relaxed">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="flex gap-4">
      <div className="flex flex-col items-center">
        <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-accent/15 border border-accent/40 text-sm font-bold text-accent">{num}</div>
        <div className="flex-1 w-px bg-border mt-2" />
      </div>
      <div className="flex-1 pb-6">
        <p className="font-bold text-base text-text-primary mb-1.5">{title}</p>
        <div className="text-sm text-text-secondary space-y-1.5 leading-relaxed">{children}</div>
      </div>
    </div>
  )
}

function BiasCard({ num, name, desc, fix }: { num: number; name: string; desc: string; fix: string }) {
  return (
    <div className="rounded-xl border border-border bg-bg-card p-5 space-y-3">
      <div className="flex items-start gap-3">
        <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-loss/15 border border-loss/30 text-sm font-bold text-loss">{num}</span>
        <h4 className="font-bold text-base text-text-primary leading-tight">{name}</h4>
      </div>
      <p className="text-sm text-text-secondary leading-relaxed pl-10">{desc}</p>
      <div className="ml-10 flex items-start gap-2.5 rounded-lg bg-profit-dim border border-profit/20 px-3 py-2.5">
        <CheckCircle2 size={15} className="text-profit flex-shrink-0 mt-0.5" />
        <p className="text-sm text-profit leading-relaxed">{fix}</p>
      </div>
    </div>
  )
}

function ConfluenceTag({ label, color = 'accent' }: { label: string; color?: 'profit' | 'loss' | 'neutral' | 'accent' }) {
  return (
    <span className={cn(
      'inline-flex items-center rounded-md px-2.5 py-1 text-sm font-semibold border',
      color === 'profit'  && 'bg-profit-dim  text-profit  border-profit/30',
      color === 'loss'    && 'bg-loss-dim    text-loss    border-loss/30',
      color === 'neutral' && 'bg-neutral-dim text-neutral border-neutral/30',
      color === 'accent'  && 'bg-accent/10   text-accent  border-accent/30',
    )}>
      {label}
    </span>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="danger" title="Principe fondamental — Aucune exception">
        Je ne trade jamais sur une intuition. Chaque trade passe par les 7 étapes obligatoires du protocole. Si une seule étape échoue → pas de trade. Même si le setup semble "parfait". L'état émotionnel est évalué <strong className="text-text-primary">avant toute ouverture de graphique</strong>.
      </Callout>

      <Callout type="warning" title="Pourquoi c'est scientifiquement fondé — Pas une opinion">
        <ul className="space-y-2">
          <li>• <strong className="text-text-primary">Edgewonk Trading Journal — 50 000 trades analysés (2024)</strong> : Win rate 23% inférieur en état anxieux. Un système à 48% de WR normal descend à 37% en état dégradé — le rendant déficitaire.</li>
          <li>• <strong className="text-text-primary">Kahneman & Tversky, "Prospect Theory" — Prix Nobel 2002</strong> : L'aversion aux pertes fait ressentir la douleur 2.25× plus intensément que le plaisir équivalent, biaisant systématiquement les décisions sous pression.</li>
          <li>• <strong className="text-text-primary">Lo & Repin, "The Psychophysiology of Real-Time Financial Risk Processing" (2002)</strong> : Les traders montrant des réponses émotionnelles intenses ont des performances significativement inférieures à ceux qui maintiennent l'équanimité.</li>
        </ul>
      </Callout>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={r.score} className={cn(
            ...
            r.color === 'profit'  && 'border-profit/20 bg-bg-card',
            r.color === 'neutral' && 'border-neutral/20 bg-bg-card',
            r.color === 'loss'    && 'border-loss/30 bg-loss-dim',
          )}>
            <MoodIcon score={r.score} size={28} />
            <div className="flex-1 min-w-0">
              <p className="font-bold text-text-primary">{r.score}/5 — {r.label}</p>
              <p className="text-sm text-text-secondary mt-0.5">{r.desc}</p>
            </div>
            <span className={cn(
              'text-sm font-bold flex-shrink-0 text-right ml-4',
              r.color === 'profit' && 'text-profit',
              r.color === 'neutral' && 'text-neutral',
              r.color === 'loss' && 'text-loss',
            )}>{r.action}</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={s.n} className="flex items-center gap-4 rounded-xl border border-border bg-bg-card px-4 py-3.5 hover:bg-bg-hover transition-colors">
            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-accent/15 border border-accent/30 font-bold text-accent">{s.n}</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="success" title="Études sur l'importance de la routine">
        Enquête 4 200 traders actifs (2023) : <strong className="text-text-primary">78% des traders profitables sur 3 ans+ ont une routine pré-session documentée</strong>. La routine élimine la "charge décisionnelle" (concept de Baumeister, 1998) et garantit un état d'esprit structuré plutôt que réactif.
      </Callout>
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="danger" title="Erreur numéro 1 des traders débutants">
        Analyser uniquement sur le 4H sans vérifier les timeframes supérieurs. Le 4H montre un rebond sur EMA 50 — mais si le weekly est en tendance baissière sous EMA 200, ce "rebond" est en réalité un pullback dans une tendance baissière macro. Tu trades <strong className="text-text-primary">contre le courant</strong>.
      </Callout>

      <Callout type="success" title={'Principe validé — "Higher Timeframe Bias" (HTF Bias)'}>
        <strong className="text-text-primary">Stan Weinstein "Secrets for Profiting in Bull and Bear Markets" (1988)</strong>, <strong className="text-text-primary">Mark Minervini "Trade Like a Stock Market Wizard" (2013)</strong>, <strong className="text-text-primary">Mark Douglas "Trading in the Zone" (2000)</strong> : Les 3 convergent sur le même principe — le timeframe supérieur définit le biais directionnel. Le timeframe inférieur fournit l'entrée précise. Ne jamais inverser cet ordre.
      </Callout>
        ...
        {[
          {
            ...
            color: 'border-profit/30 bg-profit-dim',
          },
          {
            tf: 'Daily (1D)',
          ...
          <div key={row.tf} className={cn('rounded-xl border p-5', row.color)}>
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                <p className="font-bold text-text-primary text-base">{row.tf}</p>
                <p className="text-sm text-text-secondary">{row.role}</p>
              </div>
            </div>
            <div className="mb-3">
              <p className="text-xs font-bold uppercase tracking-wide text-text-secondary mb-2">Questions à se poser</p>
            ...
            <div className="rounded-lg bg-bg-surface border border-border px-3 py-2">
              <p className="text-sm font-semibold text-text-primary">{row.decision}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <DataTable
        ...
        rows={[
          ['Alignement parfait', <span className="text-profit">✓ Haussier</span>, <span className="text-profit">✓ Haussier</span>, <span className="text-profit">✓ Signal long</span>, <span className="text-profit font-bold">LONG — Taille normale (1%)</span>],
          ['Alignement partiel', <span className="text-profit">✓ Haussier</span>, <span className="text-neutral">Neutre/range</span>, <span className="text-profit">✓ Signal long</span>, <span className="text-neutral font-semibold">LONG possible — Taille 0.5%</span>],
          ['Contre-tendance', <span className="text-loss">✗ Baissier</span>, <span className="text-neutral">Rebond</span>, <span className="text-profit">Signal long</span>, <span className="text-loss font-bold">PAS DE LONG — Tu trades contre le courant</span>],
          ['Convergence short', <span className="text-loss">✓ Baissier</span>, <span className="text-loss">✓ Baissier</span>, <span className="text-loss">✓ Signal short</span>, <span className="text-loss font-bold">SHORT — Taille normale (1%)</span>],
        ]}
      />
      ...
      <div className="rounded-xl border border-border bg-bg-card p-5 font-mono text-sm space-y-3">
        <div className="space-y-2">
          <p className="text-text-muted text-xs font-bold uppercase tracking-wide">Weekly</p>
          <p className="text-text-secondary">BTC weekly : EMA 20 {'>'} 50 {'>'} 200 → Ruban haussier. Dernier weekly a fermé au-dessus de l'EMA 20. → <span className="text-profit font-bold">Biais LONG confirmé</span></p>
        </div>
        <div className="h-px bg-border" />
        <div className="space-y-2">
          <p className="text-text-muted text-xs font-bold uppercase tracking-wide">Daily</p>
          <p className="text-text-secondary">BTC daily : Prix revient sur EMA 50 daily. RSI daily à 48. Volume profile POC daily aligné avec EMA 50. → <span className="text-accent font-bold">Zone d'intérêt identifiée</span></p>
        </div>
        <div className="h-px bg-border" />
        <div className="space-y-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="info" title="Pourquoi la market structure est fondamentale">
        La market structure est la représentation visuelle de l'offre et de la demande au fil du temps. Elle est utilisée par les desks propriétaires, les hedge funds et les market makers. <strong className="text-text-primary">C'est le "langage" du marché</strong>, indépendant de tout indicateur. Comprendre la structure te permet d'identifier si une tendance est intacte ou en train de changer — avant que les indicateurs ne le montrent.
      </Callout>

      <SubHeading icon={<TrendingUp size={18} />}>Les Bases — Swing Highs & Swing Lows</SubHeading>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-profit/30 bg-profit-dim p-4 space-y-3">
          <p className="font-bold text-profit">Uptrend — Structure Haussière</p>
          <div className="space-y-1.5 text-sm text-text-secondary">
            <p>• <strong className="text-text-primary">Higher Highs (HH)</strong> : Chaque sommet est plus haut que le précédent</p>
            <p>• <strong className="text-text-primary">Higher Lows (HL)</strong> : Chaque creux est plus haut que le précédent</p>
          </div>
          <p className="text-sm font-bold text-profit">→ Structure intacte = LONG uniquement</p>
          <div className="rounded-lg bg-bg-card border border-border px-3 py-2 font-mono text-sm text-text-secondary">
            HL1 → HH1 → HL2 → HH2 → HL3 → HH3
          </div>
        </div>
        <div className="rounded-xl border border-loss/30 bg-loss-dim p-4 space-y-3">
          <p className="font-bold text-loss">Downtrend — Structure Baissière</p>
          <div className="space-y-1.5 text-sm text-text-secondary">
            <p>• <strong className="text-text-primary">Lower Highs (LH)</strong> : Chaque sommet est plus bas que le précédent</p>
            <p>• <strong className="text-text-primary">Lower Lows (LL)</strong> : Chaque creux est plus bas que le précédent</p>
          </div>
          <p className="text-sm font-bold text-loss">→ Structure intacte = SHORT uniquement</p>
          <div className="rounded-lg bg-bg-card border border-border px-3 py-2 font-mono text-sm text-text-secondary">
      ...
      <Callout type="warning" title="Distinction critique — BOS vs ChoCH">
        Un <strong className="text-text-primary">BOS (Break of Structure)</strong> confirme la continuation de tendance. Un <strong className="text-text-primary">ChoCH (Change of Character)</strong> est le premier signal d'un potentiel retournement. Les traders d'élite distinguent systématiquement les deux.
      </Callout>
      ...
      <DataTable
        ...
        rows={[
          [
            <span className="font-bold text-profit">BOS Haussier</span>,
            ...
            <span className="text-profit">Renforce la conviction pour les longs. Setup en cours reste valide.</span>,
          ],
          [
            <span className="font-bold text-loss">BOS Baissier</span>,
            'Le prix casse et ferme en dessous du dernier Lower Low',
            'La structure baissière continue. Les vendeurs dominent.',
            <span className="text-loss">Confirme les shorts. Pas de long contre-tendance.</span>,
          ],
          [
            <span className="font-bold text-neutral">ChoCH Haussier</span>,
            ...
            <span className="text-neutral">Surveiller. Attendre confirmation (Higher Low formé + 2ème BOS haussier).</span>,
          ],
          [
            <span className="font-bold text-neutral">ChoCH Baissier</span>,
            ...
            <span className="text-neutral">Réduire les longs. SL au breakeven sur positions ouvertes. Surveiller confirmation.</span>,
          ],
        ]}
      />
      ...
      <Callout type="danger" title="Règle des 3 règles de lecture de structure">
        <div className="space-y-2">
          <p><strong className="text-text-primary">Règle 1 :</strong> Fermeture de bougie obligatoire. Un simple wick qui dépasse un niveau ne constitue PAS un BOS valide.</p>
          <p><strong className="text-text-primary">Règle 2 :</strong> Sur le timeframe où tu trades (4H) ET sur le timeframe supérieur (Daily). Un BOS haussier sur 4H mais structure baissière sur Daily = signal faible.</p>
          <p><strong className="text-text-primary">Règle 3 :</strong> Un ChoCH seul ne suffit JAMAIS à entrer. Attends toujours une structure confirmée (HH/HL formés) pour les longs ou (LH/LL formés) pour les shorts.</p>
        </div>
      </Callout>
      ...
      <Callout type="tip" title="Concept utilisé par les prop traders et market makers">
        Les <strong className="text-text-primary">stop losses de la majorité</strong> se concentrent sous les swing lows évidents (uptrend) et au-dessus des swing highs évidents (downtrend). Les institutionnels cherchent ces zones de liquidité pour entrer à meilleur prix. On appelle ça une <strong>"stop hunt"</strong> ou <strong>"liquidity grab"</strong>.
      </Callout>

      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-border bg-bg-card p-4 space-y-2">
          <p className="font-bold text-text-primary">Stop Hunt Haussier (Buy Side)</p>
          <p className="text-sm text-text-secondary">Le prix descend brièvement sous un swing low évident (déclenchant tous les SL), puis remonte immédiatement avec force au-dessus.</p>
          <p className="text-sm font-bold text-profit">Signal : si le rejet est rapide et la bougie ferme haut → signal d'achat contrarian puissant. Combine avec EMA + RSI.</p>
        </div>
        <div className="rounded-xl border border-border bg-bg-card p-4 space-y-2">
          <p className="font-bold text-text-primary">Stop Hunt Baissier (Sell Side)</p>
          <p className="text-sm text-text-secondary">Le prix monte brièvement au-dessus d'un swing high évident, puis retombe immédiatement avec force.</p>
          <p className="text-sm font-bold text-loss">Signal : si le rejet est rapide → signal de vente contrarian. Combine avec résistance + RSI {'>'} 65.</p>
        </div>
      </div>
      ...
      <DataTable
        ...
        rows={[
          ['Swing Highs/Lows historiques', 'Sommets et creux majeurs sur weekly et daily', <span className="text-profit font-bold">Très forte</span>, 'Support/Résistance principale. TP ou SL selon direction.'],
          ['Point of Control (FRVP)', 'Zone de plus grand volume échangé sur le range', <span className="text-profit font-bold">Forte</span>, 'Entrée SEULEMENT si aligné avec EMA. Seul il est insuffisant.'],
          ['EMA 200 daily', 'Moyenne mobile exponentielle 200 périodes daily', <span className="text-profit font-bold">Très forte</span>, 'Niveau institutionnel global. Frontière bull/bear macro.'],
          ['EMA 50 daily', 'Moyenne mobile exponentielle 50 périodes daily', <span className="text-profit">Forte</span>, 'Retest setup principal. Fréquent et fiable en uptrend.'],
          ['Zones de liquidité (equal highs/lows)', 'Doubles sommets/creux → stops concentrés', <span className="text-neutral">Modérée</span>, 'Anticiper la stop hunt. Ne pas mettre SL à ces niveaux évidents.'],
          ['Fibonacci 0.618 (golden ratio)', 'Retracement de Fibonacci du dernier swing', <span className="text-neutral">Modérée</span>, 'Confluence supplémentaire quand aligné avec EMA. Renforce la zone.'],
        ]}
      />
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="info" title="Pourquoi l'EMA plutôt que la SMA">
        L'EMA (Exponential Moving Average) donne un poids exponentiel plus important aux prix récents. Elle réagit plus rapidement aux changements de tendance que la SMA. <strong className="text-text-primary">Les algorithmes institutionnels et les desks prop trading utilisent l'EMA 50/200 comme niveaux de référence</strong> — ce qui crée une prophétie auto-réalisatrice partielle.
      </Callout>

      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        {[
          { title: 'Bull Market Parfait', desc: 'EMA 20 > 50 > 100 > 200 — toutes orientées à la hausse. Prix au-dessus de toutes les EMA.', rule: 'Seuls les LONGS. Structure idéale.', color: 'border-profit/30 bg-profit-dim', tcolor: 'text-profit' },
          { title: 'Bear Market Confirmé', desc: 'EMA 20 < 50 < 100 < 200 — toutes orientées à la baisse. Prix sous toutes les EMA.', rule: 'Seuls les SHORTS ou cash.', color: 'border-loss/30 bg-loss-dim', tcolor: 'text-loss' },
          { title: 'Retest (Setup principal)', desc: 'Prix revient sur EMA 50 ou 200 par le dessus en bull market. Bougie 4H fermée. Volume > 120%.', rule: 'Zone d\'entrée Long idéale — C\'est là que tu attends.', color: 'border-accent/30 bg-accent/5', tcolor: 'text-accent' },
          { title: 'No Man\'s Land', desc: 'Prix entre deux EMA sans support/résistance clair. EMA entremêlées sans ordre.', rule: 'PAS DE TRADE. Attends la réorganisation.', color: 'border-neutral/30 bg-neutral-dim', tcolor: 'text-neutral' },
        ].map((r) => (
      ...
      <DataTable
        ...
        rows={[
          [<span className="font-mono font-bold text-loss">EMA 200</span>, 'Frontière macro bull/bear. Niveau institutionnel mondial. Plus grand volume de référence.', 'Daily (obligatoire) + 4H', <span className="text-profit font-bold">#1 — Setup EMA 200 retest = le plus fort</span>],
          [<span className="font-mono font-bold text-profit">EMA 50</span>, 'Tendance intermédiaire. Zone de retest principale en bull market sain.', '4H + Daily', <span className="text-profit">#2 — Setup fréquent et fiable</span>],
          [<span className="font-mono font-bold text-neutral">EMA 100</span>, 'Support intermédiaire entre 50 et 200. Valide si 50 cassée proprement.', '4H', '#3 — Secondaire seulement'],
          [<span className="font-mono font-bold text-accent">EMA 20</span>, 'Momentum court terme. Premier signal de pression sur l\'EMA 50.', '4H', 'Momentum uniquement — pas d\'entrée directe'],
        ]}
      />
      ...
      <DataTable
        ...
        rows={[
          ...
          [<span className="font-bold text-profit">0.618 (Golden Ratio)</span>, '61.8%', '"Golden Pocket" — niveau institutionnel de référence. Zone d\'accumulation documentée.', <span className="text-profit font-bold">Forte — combiné EMA = setup élite</span>],
          [<span className="font-bold text-accent">0.705</span>, '70.5%', 'Extension du Golden Pocket. Alternative si 0.618 cassé.', <span className="text-accent">Forte</span>],
      ...
      <Callout type="success" title="Confluence Fibonacci + EMA = Setup Élite">
        Quand le niveau Fibonacci 0.618 ou 0.705 coïncide avec l'EMA 50 ou EMA 200 sur le même prix → double confluence géométrique + dynamique. <strong className="text-text-primary">C'est la zone d'entrée la plus précise du protocole.</strong> Le R/R est mécaniquement meilleur car le SL peut être serré sous le niveau fib.
      </Callout>
      ...
      <Callout type="success" title="Backtest BTC 4H 2020-2024 (Altrady Research)">
        RSI divergences + EMA confluence → <strong className="text-text-primary">68% de précision sur 4H</strong>. Sans divergence RSI, ce chiffre tombe à 54%. La divergence RSI est le signal le plus prédictif de la stratégie.
      </Callout>

      <div className="grid grid-cols-1 gap-4">
        <div className="rounded-xl border border-border bg-bg-card p-5">
          <p className="font-bold text-text-primary text-base mb-4">Les 4 Types de Divergences RSI</p>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {[
              {
              ...
              },
              {
                ...
                color: 'border-loss/30 bg-loss-dim',
                tc: 'text-loss',
              ...
              },
              {
                ...
                color: 'border-neutral/30 bg-neutral-dim',
                tc: 'text-neutral',
              ...
              <div key={d.name} className={cn('rounded-xl border p-4 space-y-2', d.color)}>
                ...
                <p className="text-sm text-text-secondary italic">{d.meaning}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-profit/20 bg-bg-card px-4 py-3">
          <p className="text-sm text-text-secondary">Règle entrée LONG — RSI au moment d'entrer</p>
          <p className="font-bold text-profit text-base">RSI &lt; 60 obligatoire</p>
          <p className="text-sm text-text-muted">RSI {'>'}  65 au moment d'entrer = acheter un actif en surachat = R/R dégradé</p>
        </div>
        <div className="rounded-xl border border-loss/20 bg-bg-card px-4 py-3">
          <p className="text-sm text-text-secondary">Règle entrée SHORT — RSI au moment d'entrer</p>
          <p className="font-bold text-loss text-base">RSI &gt; 40 obligatoire</p>
          <p className="text-sm text-text-muted">RSI {'<'} 35 au moment d'entrer short = vendre un actif en survente = R/R dégradé</p>
        </div>
      </div>
      ...
      <Callout type="tip" title="Règle FRVP non-discrétionnaire">
        Ton FRVP couvre <strong className="text-text-primary">TOUJOURS</strong> depuis le dernier ATH ou ATL de BTC jusqu'à aujourd'hui. Cette règle fixe rend le POC identique peu importe tes biais du moment — élimine le confirmation bias sur le range.
      </Callout>

      <DataTable
        headers={['Zone FRVP', 'Description', 'Utilisation dans le protocole']}
        rows={[
          [<span className="font-mono font-bold text-text-primary">POC</span>, 'Point of Control — Prix avec le plus grand volume échangé du range', 'Entrée SEULEMENT si aligné avec EMA (double confluence). Seul il est insuffisant.'],
          [<span className="font-mono font-bold text-profit">VAH</span>, 'Value Area High — borne haute de la zone de 70% du volume', 'Résistance principale → 1er Take Profit pour les longs'],
          [<span className="font-mono font-bold text-neutral">VAL</span>, 'Value Area Low — borne basse de la zone de 70% du volume', 'Support → SL de référence pour les shorts depuis POC'],
      ...
      <p className="font-bold text-text-primary mt-4">OBV — On-Balance Volume (accumulation cachée)</p>
      <DataTable
        headers={['Signal OBV', 'Interprétation', 'Décision']}
        rows={[
          ['OBV monte + Prix monte', 'Hausse confirmée. Pression acheteuse saine.', <span className="text-profit">Conforme — trade long valide</span>],
          [<span className="font-semibold text-profit">OBV monte + Prix descend/stagne</span>, 'Accumulation cachée. Des institutions achètent discrètement pendant que le prix est bas.', <span className="text-profit font-bold">Signal fort de retournement long imminent</span>],
          [<span className="font-semibold text-loss">OBV descend + Prix monte</span>, 'Distribution cachée. La hausse n\'est pas soutenue par le volume réel.', <span className="text-loss font-bold">Méfie-toi. Taille réduite ou pas de long.</span>],
          ['OBV descend + Prix descend', 'Baisse confirmée. Tendance baissière saine.', <span className="text-loss">Pas de long prématuré</span>],
        ]}
      />
      ...
      <Callout type="success" title="J. Welles Wilder Jr. — 'New Concepts in Technical Trading Systems' (1978)">
        L'ATR mesure la volatilité réelle moyenne sur N périodes, indépendamment de la direction. <strong className="text-text-primary">Un SL basé sur l'ATR s'adapte automatiquement à la volatilité actuelle</strong> du marché — contrairement à un % fixe qui peut être trop serré en période volatile ou trop large en période calme.
      </Callout>

      <div className="rounded-xl border border-accent/30 bg-accent/5 p-5 font-mono text-sm space-y-3">
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wide text-text-secondary font-bold">Configuration TradingView</p>
          <p className="text-text-primary">ATR(14) sur le graphique 4H — période standard de 14 bougies</p>
        </div>
        <div className="h-px bg-border" />
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wide text-text-secondary font-bold">Formule Stop Loss basé ATR</p>
          <p className="text-text-primary">SL Long = Prix d'entrée − (ATR × <span className="text-profit">1.5</span>)</p>
          <p className="text-text-primary">SL Short = Prix d'entrée + (ATR × <span className="text-loss">1.5</span>)</p>
        </div>
        <div className="h-px bg-border" />
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wide text-text-secondary font-bold">Exemple — BTC 4H, ATR = 800$</p>
          <p className="text-text-secondary">SL Long = Prix entrée − (800 × 1.5) = Prix entrée − 1 200$</p>
          <p className="text-text-muted text-xs mt-1">Si prix d'entrée = 65 000$ → SL = 63 800$ (distance = 1.85%)</p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="info" title="Avantage informationnel unique au crypto">
        Contrairement aux actions, la blockchain est publique. Tu peux voir ce que font les baleines, les exchanges et les institutions en temps réel. C'est un avantage informationnel qui n'existe dans aucun autre marché. <strong className="text-text-primary">Les indicateurs on-chain sont des données primaires</strong> — pas des dérivés de prix comme les indicateurs techniques.
      </Callout>
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={item.name} className="rounded-xl border border-border bg-bg-card p-5">
            <p className="font-bold text-text-primary text-base mb-1">{item.name}</p>
            <p className="text-sm text-text-muted mb-3 italic">{item.how}</p>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
              <div className="rounded-lg bg-profit-dim border border-profit/20 px-3 py-2.5">
                <p className="text-xs font-bold text-profit mb-1 uppercase tracking-wide">Signal Long OK</p>
                <p className="text-sm text-text-secondary">{item.ok}</p>
              </div>
              <div className="rounded-lg bg-loss-dim border border-loss/20 px-3 py-2.5">
                <p className="text-xs font-bold text-loss mb-1 uppercase tracking-wide">Signal de Prudence</p>
                <p className="text-sm text-text-secondary">{item.bad}</p>
              </div>
              <div className="rounded-lg bg-bg-surface border border-border px-3 py-2.5">
                <p className="text-xs font-bold text-accent mb-1 uppercase tracking-wide">Pourquoi ça compte</p>
      ...
      <Callout type="warning" title="Fréquence de consultation obligatoire — Hebdomadaire uniquement">
        Ces indicateurs se mesurent sur des semaines et des mois. Les checker quotidiennement est contre-productif et source de sur-réaction aux bruit court terme. <strong className="text-text-primary">Une fois le dimanche, intégrés dans la revue hebdomadaire.</strong>
      </Callout>
        ...
        {[
          ...
          <div key={item.name} className="rounded-xl border border-border bg-bg-card p-5">
            <div className="flex items-start justify-between gap-3 mb-1">
              ...
              <span className="text-xs text-text-muted flex-shrink-0">{item.source}</span>
            </div>
            <p className="text-sm text-text-muted italic mb-3">{item.def}</p>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              <div className="rounded-lg bg-profit-dim border border-profit/20 px-3 py-2.5">
                <p className="text-xs font-bold text-profit mb-1.5 uppercase tracking-wide">Signal Long Fort</p>
                <p className="text-sm text-text-secondary leading-relaxed">{item.long}</p>
              </div>
              <div className="rounded-lg bg-loss-dim border border-loss/20 px-3 py-2.5">
                <p className="text-xs font-bold text-loss mb-1.5 uppercase tracking-wide">Signal de Prudence</p>
                <p className="text-sm text-text-secondary leading-relaxed">{item.caution}</p>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="info" title="Qu'est-ce qu'Arkham Intelligence ?">
        Arkham identifie {'>'} 800M d'adresses liées à {'>'} 450 000 entités (2025). Il transforme des adresses anonymes en information actionnable sur qui fait quoi. <strong className="text-text-primary">Règle absolue : Arkham ne déclenche jamais un trade seul.</strong> Il filtre ou invalide un setup technique existant.
      </Callout>
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={alert.type} className="rounded-xl border border-border bg-bg-card p-5 space-y-3">
            <p className="font-bold text-text-primary text-base">{alert.type}</p>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div className="space-y-1">
                <p className="text-xs font-bold uppercase tracking-wide text-text-secondary">Ce que ça signifie</p>
                <p className="text-sm text-text-secondary leading-relaxed">{alert.meaning}</p>
              </div>
              <div className="rounded-lg bg-bg-surface border border-border px-3 py-2.5 space-y-1">
                <p className="text-xs font-bold uppercase tracking-wide text-text-secondary">Action recommandée</p>
...
function TabExecution() {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {[
          { type: 'Ordre Limite', usage: 'TOUTES tes entrées', why: 'Zéro slippage. Frais maker (moins chers). Prix exact choisi à l\'avance. Sur 5x levier, 0.5% de slippage = 2.5% de perte avant de commencer.', color: 'border-profit/30 bg-profit-dim', tc: 'text-profit', icon: <CheckCircle2 size={18} className="text-profit" /> },
          { type: 'Stop Market', usage: 'Ton Stop Loss UNIQUEMENT', why: 'Exécution garantie même en flash crash. Mieux vaut 0.1% de slippage qu\'une liquidation totale. Le SL en Stop Market est obligatoire.', color: 'border-neutral/30 bg-neutral-dim', tc: 'text-neutral', icon: <CheckCircle2 size={18} className="text-neutral" /> },
          { type: 'Ordre Market', usage: 'JAMAIS pour l\'entrée', why: 'Slippage 0.1-0.5% + frais taker (1.5-3× plus chers). Sur 100 trades, la différence de rentabilité est significative. Aucune justification valable.', color: 'border-loss/30 bg-loss-dim', tc: 'text-loss', icon: <XCircle size={18} className="text-loss" /> },
        ].map((r) => (
          <div key={r.type} className={cn('rounded-xl border p-4', r.color)}>
            ...
            <p className="text-sm text-text-secondary">{r.why}</p>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {[
          {
            rank: '1', label: 'Retest EMA 200', color: 'border-profit/40 bg-profit-dim', tc: 'text-profit',
          ...
          {
            rank: '2', label: 'Retest EMA 50', color: 'border-accent/30 bg-accent/5', tc: 'text-accent',
          ...
          {
            rank: '3', label: 'Retest POC + EMA (confluence)', color: 'border-border bg-bg-card', tc: 'text-text-secondary',
      ...
      <div className="rounded-xl border border-accent/30 bg-accent/5 p-5 font-mono space-y-4">
        {[
          ...
          <div key={r.step} className="space-y-1">
            <p className="text-xs uppercase tracking-wide text-text-secondary font-bold">Étape {r.step} — {r.label}</p>
            <p className="text-base text-text-primary">{r.formula}</p>
            <p className="text-sm text-text-muted">{r.ex}</p>
            {r.step !== '4' && <div className="h-px bg-border mt-3" />}
          </div>
        ))}
      </div>

      <p className="text-sm font-bold text-text-primary">Table de référence — Capital 10 000$, Risque 1% = 100$</p>
      <DataTable
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={n} className={cn('flex items-start gap-3 rounded-xl border px-4 py-3', critical ? 'border-accent/30 bg-accent/5' : 'border-border bg-bg-surface')}>
            <span className="font-mono text-sm text-text-muted flex-shrink-0 w-6">{n}</span>
      ...
      <DataTable
        ...
        rows={[
          [<span className="font-semibold text-loss">Entrer au Market</span>, 'Slippage + frais taker = 1-3% de perte avant le début du trade', 'Toujours Limit. Si le prix part sans toi → ce n\'était pas ton prix.'],
          [<span className="font-semibold text-loss">SL trop serré ({'<'} ATR)</span>, 'BTC fait des ±0.5-1% normalement → sorti avant le mouvement', 'SL = ATR × 1.5 ou minimum sous la structure technique'],
          [<span className="font-semibold text-loss">SL posé après entrée</span>, 'Flash crash 3% en 90 secondes = liquidation sans SL', 'TP + SL posés EN MÊME TEMPS que l\'ordre d\'entrée'],
          [<span className="font-semibold text-loss">TP au-delà d\'une résistance</span>, 'Prix bloque à la résistance. Tu refuses de fermer. Retournement.', 'TP toujours AVANT la prochaine résistance identifiable'],
          [<span className="font-semibold text-loss">Averaging Down</span>, 'Martingale = ruine mathématique. 1 → 2 → 4 → liquidation.', '1 trade = 1 entrée. SL touché = trade terminé.'],
          [<span className="font-semibold text-loss">Déplacer SL en perte</span>, 'Perte -1% → -3% → -8% → liquidation. Documenté des milliers de fois.', 'SL ne se déplace JAMAIS dans la direction adverse. JAMAIS.'],
        ]}
      />
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="success" title="Pourquoi la gestion post-entrée est aussi importante que l'entrée elle-même">
        <strong className="text-text-primary">Linda Bradford Raschke (Market Wizards, Schwager 1992)</strong> : "Entry is 20% of the trade. Management is 80%." Les traders amateurs fixent leur sortie et n'y touchent plus. Les traders professionnels gèrent activement leur position selon des règles prédéfinies.
      </Callout>
      ...
      <div className="space-y-3">
        {[
          ...
          },
          {
            ...
            color: 'border-profit/20 bg-bg-card',
            tc: 'text-profit',
          },
          {
          ...
          },
          {
            ...
            color: 'border-profit/40 bg-profit-dim',
            tc: 'text-profit',
          },
        ].map((s, i) => (
          <div key={i} className={cn('rounded-xl border p-5 space-y-2', s.color)}>
            <div className="flex items-center gap-2">
              <span className={cn('flex h-7 w-7 items-center justify-center rounded-full bg-accent text-white text-sm font-bold')}>{i + 1}</span>
              <p className={cn('font-bold text-base', s.tc)}>{s.milestone}</p>
            </div>
            <p className="text-sm font-bold text-text-primary pl-9">{s.action}</p>
            <p className="text-sm text-text-secondary pl-9">{s.why}</p>
          </div>
        ))}
      </div>
      ...
      <Callout type="tip" title="Exemple concret — BTC Long à 65 000$, risque 1R = 1 000$">
        <div className="font-mono text-sm space-y-1.5 mt-1">
          ...
          <p className="text-accent">@ 66 200$ (+1R) → SL déplacé à 65 000$ (breakeven)</p>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={i} className="rounded-xl border border-loss/20 bg-loss-dim p-4">
            <div className="flex items-start gap-2">
              <XCircle size={16} className="text-loss flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-bold text-text-primary">{r.rule}</p>
                <p className="text-sm text-text-secondary mt-1">{r.detail}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <Callout type="info" title="Études sur le feedback loop en trading">
        <strong className="text-text-primary">Brett Steenbarger "Enhancing Trader Performance" (2007)</strong> : Les traders qui analysent systématiquement chaque trade (WON ou LOST) progressent 3× plus vite que ceux qui ne le font pas. L'objectif n'est pas de critiquer — c'est d'identifier des patterns.
      </Callout>

      <div className="rounded-xl border border-border bg-bg-card p-5 space-y-3">
        <p className="font-bold text-text-primary">Pour chaque trade fermé — Réponds à ces questions dans le journal</p>
        <div className="space-y-2">
          {[
      ...
      <Callout type="success" title="Mathématique du plan de gestion — Pourquoi ça change tout">
        <div className="font-mono text-sm space-y-2 mt-1">
          <p className="text-text-primary font-bold">Scénario A — Gestion passive (fermer au TP ou SL) : Win Rate 45%, R/R 1:2.5</p>
          <p className="text-text-secondary">(45 × 2.5R) − (55 × 1R) = 112.5 − 55 = <span className="text-profit font-bold">+57.5R sur 100 trades</span></p>
          <div className="h-px bg-border" />
          <p className="text-text-primary font-bold">Scénario B — Gestion active (breakeven + TP partiel) : Win Rate 45%, R/R moyen 2R</p>
          <p className="text-text-secondary">(45 × 2R) − (45 × 0R breakeven) − (10 × 1R vraies pertes) = 90 − 10 = <span className="text-profit font-bold">+80R sur 100 trades</span></p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="info" title="Fréquence et rôle">
        Ce check se fait <strong className="text-text-primary">une fois par semaine (dimanche)</strong>. Il définit le contexte macro pour toute la semaine. Il ne génère pas de signal de trade en lui-même — il définit si les conditions sont favorables et quelle taille utiliser.
      </Callout>
      ...
      <DataTable
        ...
        rows={[
          ['> 55% et montant', <span className="text-neutral">BTC domine — altcoins sous pression</span>, 'Trader uniquement BTC. ETH/SOL : taille réduite 50% ou pas de trade.'],
          ['50-55% stable', 'Marché équilibré', 'Trading normal sur tous les actifs'],
          ['< 50% et descendant', <span className="text-profit">Altcoin season potentiel</span>, 'ETH et SOL peuvent fortement surperformer BTC. Opportunités amplifiées.'],
          ['BTC.D chute brusque de 3%+ en 1 semaine', <span className="text-profit">Rotation rapide vers altcoins</span>, 'Signal fort de surperformance ETH/SOL à court terme'],
        ]}
      />
      ...
      <DataTable
        ...
        rows={[
          ['Baisse de taux > 50% prob.', <span className="text-profit">Dovish — favorable actifs risqués</span>, 'Trade normalement, taille normale'],
          ['Statu quo attendu', 'Neutre', 'Trade normalement'],
          ['Hausse de taux > 50% prob.', <span className="text-neutral">Hawkish — pression crypto</span>, <span className="text-neutral font-semibold">Taille réduite 30%</span>],
          ['FOMC dans les 48h', <span className="text-loss">Volatilité imprévisible</span>, <span className="text-loss font-semibold">Pas de nouveau trade. SL au breakeven.</span>],
        ]}
      />
      ...
      <DataTable
        ...
        rows={[
          ['DXY sous EMA 50 et en baisse', <span className="text-profit">Dollar faible → vents porteurs</span>, 'Trade normalement'],
          ['DXY au-dessus EMA 50 et monte', <span className="text-neutral">Dollar fort → vent de face</span>, 'Plus sélectif sur les longs'],
          ['DXY casse EMA 200 vers le bas', <span className="text-profit font-semibold">Signal haussier durable</span>, 'Conditions excellentes pour longs'],
      ...
      <DataTable
        ...
        rows={[
          [<strong className="text-text-primary">FOMC Meeting + Rate Decision</strong>, '8×/an', <span className="text-loss font-semibold">Pas de trade 48h avant ET après</span>],
          [<strong className="text-text-primary">Fed Chair Press Conference</strong>, 'Après chaque FOMC', <span className="text-loss font-semibold">Identique FOMC — Jay Powell peut retourner un marché en une phrase</span>],
          [<strong className="text-text-primary">CPI</strong>, 'Mensuel', <span className="text-neutral">Pas de nouveau trade ce jour — SL au breakeven si position ouverte</span>],
        ...
        {[
          {
            ...
            rows: [
              { v: '< 12', z: 'Complaisance', a: 'Prudence longs', c: 'text-neutral' },
              { v: '12–20', z: 'Normal', a: 'Trade librement', c: 'text-profit' },
              { v: '20–30', z: 'Tension', a: 'Taille −25%', c: 'text-neutral' },
              { v: '> 30', z: 'Peur', a: 'Zone contrarian', c: 'text-profit' },
              { v: '> 50', z: 'Panique', a: 'Opportunité rare', c: 'text-profit' },
            ],
          },
          {
            title: 'US10Y (TradingView : US10Y)',
            rows: [
              { v: '< 4.0%',   z: 'Favorable',  a: 'Normal',             c: 'text-profit' },
              { v: '4.0–4.5%', z: 'Neutre',     a: 'Normal',             c: 'text-text-secondary' },
              { v: '4.5–5.0%', z: 'Pression',   a: 'Vigilance',          c: 'text-neutral' },
              { v: '> 5.0%',   z: 'Hostile',    a: 'Réduis exposition',  c: 'text-loss' },
            ],
          },
          {
            title: 'Fear & Greed',
            rows: [
              { v: '0–25',   z: 'Peur extrême', a: 'Accumulation historique', c: 'text-profit' },
              { v: '26–59',  z: 'Peur → Neutre', a: 'Normal',                c: 'text-text-secondary' },
              { v: '60–75',  z: 'Avidité',       a: 'Vigilance',             c: 'text-neutral' },
              { v: '76–100', z: 'Avidité extrême', a: 'Prudence maximale',   c: 'text-loss' },
            ],
          },
        ].map((card) => (
          <div key={card.title} className="rounded-xl border border-border bg-bg-card p-4">
            <p className="font-bold text-text-primary mb-3">{card.title}</p>
            <div className="space-y-1.5">
              {card.rows.map(({ v, z, a, c }) => (
                <div key={v} className="flex items-center justify-between text-sm border-b border-border last:border-0 pb-1.5 last:pb-0">
                  <span className="font-mono text-text-secondary w-16 flex-shrink-0">{v}</span>
                  <span className="text-text-secondary flex-1 mx-2 text-xs">{z}</span>
    ...
    <div className="space-y-6">
      ...
      <Callout type="success" title="Études de référence — Base scientifique">
        <ul className="space-y-1.5">
          <li>• <strong className="text-text-primary">Kahneman & Tversky — Prix Nobel 2002</strong> : Aversion aux pertes = 2.25× le plaisir équivalent</li>
          <li>• <strong className="text-text-primary">Barber & Odean (2000) — 66 000 comptes</strong> : Le day trading perd systématiquement. Le swing trend-following conserve l'edge.</li>
          <li>• <strong className="text-text-primary">Edgewonk 50 000 trades (2024)</strong> : Trades en état anxieux = WR 23% inférieur</li>
          <li>• <strong className="text-text-primary">Van Tharp (2008) — 500 traders</strong> : 94% des survivants 5 ans+ risquent ≤ 2% par trade</li>
          <li>• <strong className="text-text-primary">Lo & Repin (2002), MIT</strong> : Les réponses émotionnelles intenses = performances inférieures</li>
          <li>• <strong className="text-text-primary">Steenbarger (2007)</strong> : Journal + analyse post-trade = progression 3× plus rapide</li>
        </ul>
      </Callout>
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
        {[
          ...
          <div key={s} className="flex items-start gap-3 rounded-xl border border-loss/20 bg-loss-dim px-4 py-3">
            <AlertOctagon size={15} className="text-loss flex-shrink-0 mt-0.5" />
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={p.n} className="flex gap-4 rounded-xl border border-border bg-bg-card p-4">
            <span className="flex-shrink-0 font-mono text-base font-bold text-accent">{p.n}</span>
            <div><p className="font-bold text-text-primary">{p.t}</p><p className="text-sm text-text-secondary mt-1.5 leading-relaxed">{p.d}</p></div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {[
          ...
          const sevMap: Record<string, { tag: string; label: string }> = { medium: { tag: 'bg-neutral/20 text-neutral border-neutral/40 bg-neutral-dim', label: 'MODÉRÉ' }, high: { tag: 'border-loss/30 bg-loss-dim', label: 'ÉLEVÉ' }, critical: { tag: 'border-loss/40 bg-loss-dim', label: 'CRITIQUE' } }
          const sev = sevMap[cb.sev] ?? sevMap['medium']
          return (
            <div key={cb.cond} className={cn('rounded-xl border p-4 space-y-2', sev.tag)}>
              <div className="flex items-start justify-between gap-3">
                <p className="font-bold text-text-primary">{cb.cond}</p>
                <span className={cn('flex-shrink-0 rounded-md px-2 py-0.5 text-xs font-bold', cb.sev === 'critical' ? 'bg-loss text-white' : cb.sev === 'high' ? 'bg-loss/20 text-loss' : 'bg-neutral/20 text-neutral')}>{sev.label}</span>
              </div>
              <p className="text-sm font-semibold text-loss flex items-center gap-2"><ArrowRight size={14} className="flex-shrink-0" />{cb.action}</p>
              <p className="text-sm text-text-secondary italic">{cb.why}</p>
            </div>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {[
          { color: 'border-profit/30 bg-profit-dim', icon: <CheckCircle2 size={18} className="text-profit" />, iconBg: 'bg-profit', label: 'VERT — 1%', items: ['5+ filtres alignés', 'État émotionnel ≥ 3/5', 'Pas d\'événement macro 48h', 'VIX < 25', 'Structure MTF alignée'], tc: 'text-profit' },
          { color: 'border-neutral/30 bg-neutral-dim', icon: <AlertTriangle size={18} className="text-neutral" />, iconBg: 'bg-neutral', label: 'JAUNE — 0.5%', items: ['3-4 filtres alignés', 'VIX 25-35', 'Contexte macro incertain', 'MTF partiellement aligné'], tc: 'text-neutral' },
          { color: 'border-loss/30 bg-loss-dim', icon: <XCircle size={18} className="text-loss" />, iconBg: 'bg-loss', label: 'ROUGE — Pas de trade', items: ['< 3 filtres alignés', 'État émotionnel ≤ 2/5', 'FOMC/CPI dans 48h', 'No Man\'s Land EMA', 'Circuit-breaker actif'], tc: 'text-loss' },
        ].map((s) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <Callout type="success" title="Sources — arxiv 2501.09911 (2025) + CME Group + Stoic.ai (2026)">
        Le NASDAQ 100 est l'actif le plus corrélé à BTC avec une corrélation atteignant <strong className="text-text-primary">0.87 à 0.92</strong>. BTC se comporte comme un "NASDAQ leveragé" : même direction générale, amplitude 3-5× supérieure.
      </Callout>

      <DataTable
        headers={['Actif', 'Corrélation avec BTC', 'Nature de la relation']}
        rows={[
          [<strong className="text-text-primary">NASDAQ 100 (QQQ)</strong>, <span className="text-profit font-bold">0.76 à 0.92 — #1</span>, 'Relation la plus forte. BTC = NASDAQ leveragé. 60%+ de tech dans l\'indice. Même profil d\'investisseur.'],
          [<strong className="text-text-primary">S&P 500 (SPX/SPY)</strong>, <span className="text-profit">0.70 à 0.87 — #2</span>, 'Forte depuis ETF BTC jan 2024. Renforcée par les flux institutionnels communs.'],
          [<strong className="text-text-primary">DXY</strong>, <span className="text-loss">-0.65 à -0.75 — inverse</span>, 'Dollar fort = pression BTC. Dollar faible = vents porteurs crypto.'],
          [<strong className="text-text-primary">US Treasuries (TLT)</strong>, 'Faible à négative', 'Risk-off (crise) : vente BTC + achat bonds. En régime normal : peu de corrélation.'],
        ]}
      />

      <SubHeading icon={<BarChart2 size={18} />}>Données chiffrées historiques</SubHeading>
      <div className="rounded-xl border border-border bg-bg-card p-5 font-mono">
        <div className="grid grid-cols-4 gap-4 text-sm border-b border-border pb-3 mb-3">
          <span className="text-text-muted font-bold">Année</span>
          <span className="text-text-secondary font-bold">NASDAQ</span>
          <span className="text-text-secondary font-bold">BTC</span>
          <span className="text-accent font-bold">Ratio</span>
        </div>
        {[
          ...
          <div key={y} className="grid grid-cols-4 gap-4 text-sm py-2 border-b border-border last:border-0">
            <span className="text-text-muted">{y}</span>
            <span className={n.startsWith('+') ? 'text-profit' : 'text-loss'}>{n}</span>
            <span className={b.startsWith('+') ? 'text-profit font-bold' : 'text-loss font-bold'}>{b}</span>
      ...
      <Callout type="warning" title="Limite — Corrélation asymétrique crisis vs bull">
        La corrélation est <strong className="text-text-primary">plus forte en risk-off (crise)</strong>. En régime de crise, BTC et NASDAQ chutent ensemble (corrélation 0.90+). En régime haussier, BTC peut surperformer ou sous-performer selon les dynamiques crypto-natives. Le NASDAQ est un filtre de contexte, pas une règle mécanique.
      </Callout>
      ...
      <DataTable
        ...
        rows={[
          [<span className="text-profit font-bold">QQQ au-dessus EMA 200 weekly</span>, 'Tendance macro favorable pour actifs risqués — conditions normales', 'Trade normalement'],
          [<span className="text-neutral font-bold">QQQ frôle EMA 200 weekly (±2%)</span>, 'Zone de décision critique — macro incertaine', 'Taille réduite 30%. Trades uniquement si confluence maximale.'],
          [<span className="text-loss font-bold">QQQ sous EMA 200 weekly</span>, 'Tendance macro négative — pression institutionnelle sur tout actif risqué', <span className="text-loss font-bold">Très sélectif. Taille réduite 50%. Favorise CASH ou shorts.</span>],
        ]}
      />
      ...
      <Callout type="info" title="QuantifiedStrategies — analyse comparative 2024-2026">
        Le Williams %R est légèrement supérieur au RSI pour le swing trading sur les <strong className="text-text-primary">indices (SPX, QQQ)</strong>. Sur BTC/ETH/SOL, le RSI reste la référence.
      </Callout>
      <DataTable
        headers={['W%R', 'Zone', 'Signal SPX/QQQ']}
        rows={[
          ['0 à -20', <span className="text-loss">Surachat</span>, 'Prudence sur nouveaux longs. Potentiel de correction.'],
          ['-20 à -50', 'Normale haussière', 'Pas de biais.'],
          ['-50 à -80', 'Normale baissière', 'Pas de biais.'],
          ['-80 à -100', <span className="text-profit">Survente</span>, 'Signal long potentiel si structure technique confirme.'],
        ]}
      />
    </div>
  )
...
export default function ProtocolPage() {
  ...
  return (
    <div className="min-h-screen animate-fade-in">
      {/* Header */}
      <div className="border-b border-border bg-bg-surface px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 rounded-xl border border-accent/20 bg-accent/5 px-4 py-2.5">
            <Lock size={14} className="text-accent" />
            <span className="text-sm font-semibold text-accent">Protocole verrouillé</span>
          </div>
        </div>
      </div>
      ...
      <div className="border-b border-border bg-bg-surface">
        <div className="flex overflow-x-auto px-6">
          {TABS.map(({ id, label, icon: Icon }) => (
            ...
            >
              <Icon size={15} className={activeTab === id ? 'text-accent' : 'text-text-muted'} />
              {label}
            </button>
          ))}
        </div>
      ...
      {/* Footer */}
      <div className="mx-6 mb-8 rounded-xl border border-accent/20 bg-accent/5 px-6 py-5">
        <p className="text-sm font-semibold text-accent mb-1">Protocole Swing Trading 4H — Version 2.0</p>
