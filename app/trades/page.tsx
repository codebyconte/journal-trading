...
import { Plus, RefreshCw, Target, TrendingUp, Shield, AlertTriangle, BookOpen } from 'lucide-react'
...
import { PRESET_ASSETS, type Trade, type Settings } from '@/lib/types'
...
export default function TradesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!tradesRes.ok) throw new Error(tradesJson.error ?? 'Erreur trades')
      setTrades(Array.isArray(tradesJson?.trades) ? tradesJson.trades : [])
      setSettings(settingsRes.ok ? settingsJson : null)
    } catch (e) {
  ...
  const handleOpen = async (trade: Trade) => {
    await fetch(`/api/trades/${trade.id}`, {
      ...
      body: JSON.stringify({ status: 'OPEN', openedAt: new Date().toISOString() }),
    })
    load()
  }
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in max-w-7xl">
      ...
      {error && (
        <div className="flex items-start gap-3 rounded-xl border border-loss/30 bg-loss-dim px-4 py-3">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">
        {[
          ...
          { label: 'Capital', value: formatCurrency(settings?.currentCapital ?? 0), color: 'text-text-primary' },
      ...
      >
        <TradeForm
          currentCapital={settings?.currentCapital ?? 100000}
