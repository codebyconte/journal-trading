  ...
  const load = useCallback(async () => {
    ...
    try {
      const [sr, tr] = await Promise.all([
        fetch('/api/stats'),
        fetch('/api/trades?limit=8&status=CLOSED'),
      ])
      const sd = await sr.json()
      const td = await tr.json()

      if (!sr.ok || sd.error) throw new Error(sd.error ?? 'Erreur stats')
      setStats(sd)
      setRecentTrades(Array.isArray(td.trades) ? td.trades : [])
    } catch (e) {
  ...
  if (error || !stats) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4 p-6">
        ...
        <p className="text-text-secondary">{error ?? 'Impossible de charger le dashboard'}</p>
        <button onClick={load} className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-sm font-semibold text-white">
    ...
    <div className="p-6 space-y-6 animate-fade-in max-w-7xl">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={load}
            className="flex items-center gap-2 rounded-xl border border-border bg-bg-card px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover"
          >
            <RefreshCw size={14} />
          </button>
        </div>
